...
下表总结了各测试方向上Trigger完全按下时的典型相对位移数据（VR世界坐标系）：

| 测试方向 | 相对位移 X (mm) | 相对位移 Y (mm) | 相对位移 Z (mm) | 总位移 (mm) | 主要运动轴 |
|---------|----------------|----------------|----------------|------------|-----------|
| **FORWARD** | +0.029 | -0.178 | +0.088 | ~0.20 | Y轴 (负向) |
| **BACKWARD** | -0.214 | +0.617 | -0.121 | ~0.66 | Y轴 (正向) |
| **LEFT** | +1.020 | -0.219 | +0.203 | ~1.05 | X轴 (正向) |
| **RIGHT** | -1.267 | +0.059 | +0.070 | ~1.27 | X轴 (负向) |
| **UP** | +0.723 | +0.332 | +0.120 | ~0.80 | X轴 & Y轴 |
| **DOWN** | +0.965 | +0.558 | -0.346 | ~1.15 | X轴 & Y轴 |

**关键发现**：

1. **位移量级**: 所有相对位移均在 **0.2-1.3 毫米** 范围内，属于手持抖动的自然噪音级别

2. **坐标轴映射** (基于实测数据推断):
   - **LEFT/RIGHT 测试**: 主要产生 **X轴** 位移（LEFT为正，RIGHT为负）
   - **FORWARD/BACKWARD 测试**: 主要产生 **Y轴** 位移（BACKWARD为正，FORWARD为负）
   - **UP/DOWN 测试**: 产生 **X、Y、Z轴** 混合位移

3. **测试方法限制**:
   - 用户在按住Trigger时**保持手柄相对静止**
   - 未进行**主动移动测试**（按住Trigger后移动手柄）
   - 当前数据仅反映手持稳定性，无法验证动态运动时的坐标系转换

#### 5.2.2 FORWARD方向详细数据

从日志中提取的FORWARD方向时间序列数据：

```
时间段: 03:27:04.801 - 03:27:04.886 (85ms, 6个采样)
测试方向: FORWARD (Trigger逐渐按下: 0.67 → 0.95)

相对位置序列:
1. x=0.000005, y=-0.000085, z=0.000041   (位移: 0.096mm, Trigger: 0.67)
2. x=0.000012, y=-0.000139, z=0.000067   (位移: 0.156mm, Trigger: 0.83)
3. x=0.000021, y=-0.000166, z=0.000081   (位移: 0.185mm, Trigger: 0.89)
4. x=0.000029, y=-0.000178, z=0.000088   (位移: 0.200mm, Trigger: 0.94)
5. x=0.000043, y=-0.000181, z=0.000090   (位移: 0.208mm, Trigger: 0.95)
```

**观察**:
- 随着Trigger按压深度增加，相对位移逐渐累积
- 主要位移在Y轴负方向 (约0.18mm)
- X轴和Z轴位移较小 (<0.1mm)
- 位移速度: ~2.5 mm/s (极慢，说明手柄基本静止)

#### 5.2.3 分析结论

**当前测试数据的局限性**:
- ❌ 未测试"按住Trigger后主动移动手柄"的场景
- ❌ 无法验证大幅度运动时的坐标系转换正确性
- ❌ 无法确认相对位移是否正确映射到手柄本地坐标系
- ✅ 仅验证了静止状态下的基准姿态记录功能

**建议的补充测试**:
1. 按住Trigger后，沿手柄前方推进/拉回 10-20cm
2. 按住Trigger后，左右横向移动 10-20cm
3. 按住Trigger后，上下垂直移动 10-20cm
4. 记录并分析相对位移是否符合预期方向和幅度
...
基于对 `controller_debug_20260116_032700.log` (92,264行, 2.2MB) 的详细分析：

**测试覆盖情况**:
- ✅ 6个方向完整测试: FORWARD, BACKWARD, LEFT, RIGHT, UP, DOWN
- ✅ 4,845次采样记录，覆盖各种按钮状态
- ❌ 所有测试中手柄保持静止（相对位移 < 1.3mm）
- ❌ 缺少主动运动测试（按住Trigger后大幅移动手柄）

**坐标轴映射发现** (基于静态测试):
```
VR世界坐标系映射关系（推测）:
- 手柄 LEFT/RIGHT  → 主要影响 X轴 (±1.0-1.3mm)
- 手柄 FORWARD/BACKWARD → 主要影响 Y轴 (±0.2-0.6mm)
- 手柄 UP/DOWN → 影响 X、Y、Z轴的组合 (~0.8-1.2mm)
```

**测试结论**:
1. 相对姿态计算逻辑正常工作（在静态场景下）
2. 初始姿态记录功能正确（Trigger按下时锁定基准）
3. 当前测试数据**不足以验证**动态运动时的坐标系转换
4. 无法判断是否需要从世界坐标系转换到手柄本地坐标系
...
4. **缺少动态验证**: 需要补充"按住Trigger后主动移动"的测试场景
